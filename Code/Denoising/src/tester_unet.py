...
from ThermalDenoising.model.UnetDPM import UnetDPM, EMA
...
class Tester:
    def __init__(self, config):
        ...
        self.model_type = getattr(config, 'MODEL_TYPE', 'NAFNET')
    ...
    def test(self):
        with torch.no_grad():
            ...
            for img, gt, name in tq:
                ...
                # Save comparison image (noisy input, clean GT, initial prediction, final denoised)
            ...
            self.logger.info(
                ...
                f"Thermal denoising test ({self.model_type})", ave_psnr, ave_ssim
            ))
