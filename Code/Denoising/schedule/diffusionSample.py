...
class GaussianDiffusion(nn.Module):
    ...
    def noisy_image(self, t, y):
        """ 
        Add noise to the input image according to the diffusion process
        
        Args:
            t: Timestep
            y: Clean image/residual
            
        Returns:
            y_noisy: Noisy version of y at timestep t
    ...
    def forward(self, x_T, cond, pre_ori='False'):
        """
        Algorithm 2: Sampling from the diffusion model
        
        Args:
            x_T: Initial noise
            cond: Conditioning signal (initial prediction)
            pre_ori: Whether to predict the original image directly
            
        Returns:
            x_0: Predicted clean residual
        """
        x_t = x_T
        cond_ = cond
