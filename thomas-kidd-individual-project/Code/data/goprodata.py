...
class GoProData(Dataset):
    def __init__(self, base_path, split='train', loadSize=(256, 256), mode=1, grayscale=False, use_gamma=False, max_images=None):
        """
        ...
        Args:
            ...
            max_images: Maximum number of images to use (useful for testing with a subset)
        """
        super().__init__()
        ...
        if max_images is not None and max_images > 0 and len(self.blur_images) > max_images:
            # Take a random subset to ensure diverse images
            indices = list(range(len(self.blur_images)))
            random.seed(42)  # For reproducibility
            random.shuffle(indices)
            selected_indices = indices[:max_images]
            
            self.blur_images = [self.blur_images[i] for i in selected_indices]
            self.sharp_images = [self.sharp_images[i] for i in selected_indices]
            self.scene_names = [self.scene_names[i] for i in selected_indices]
            
            print(f"Limiting dataset to {max_images} images out of {len(indices)}")
